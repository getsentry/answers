...
export const wordmarkWidth = 200;

export const glyphWidth = 50;

export const logoHeight = 44;

export const clearSpace = logoHeight / 4;

export const geometry = (opts = {}) => {
  const { style = 'wordmark', clearSpace = 0 } = opts;

  const width = style === 'wordmark' ? wordmarkWidth : glyphWidth;
  const height = logoHeight;
  const ratio = (height + clearSpace * 2) / (width + clearSpace * 2);
  const viewbox = `0 0 ${width + clearSpace * 2} ${height + clearSpace * 2}`;
...
const Logo = () => {
  return (
    <>
      ...
      >
        <path fill="currentColor" d={`${textPath} ${glyphPath}`} />
      </StyledLogotype>
    </>
  );
};
